%% Data Loading - 10 class problem
clc; clear;
data = imageDatastore("/exports/eddie/scratch/s2190468/capture24/data/GAF_ten/", "IncludeSubfolders",true);
%retrieve file names
...
% oldFcName = lgraph.Layers(fcIdx).Name;
% 
% disp(['Found old FC layer: ', oldFcName]);
% 
% numClasses = 10;
% newFc = fullyConnectedLayer(numClasses, 'Name', 'rcnnFC_10', 'WeightsInitializer', 'he');
% 
...
% start training 
% numClasses = 10;
% labelCounts = countcats(train.Labels);
% totalImages = sum(labelCounts);
% 
% % Calculate weights
% classWeights = totalImages ./ (numClasses * labelCounts);
% classWeights = single(classWeights');
% 
% disp('Class Weights (Alphabetical Order):');
% disp(classWeights);
% 
% weightedLossResNet = @(Y,T) crossentropy(Y, T, 'Weights', classWeights, 'WeightsFormat', 'C');
% % start training 
% [ResNet_FineTuned, infoResNet] = trainnet(train,net,weightedLossResNet, options);
load('ResNet_Trained_Model_ten.mat', 'ResNet_FineTuned')
...
% save('ResNet_Trained_Model_ten.mat', 'ResNet_FineTuned', '-v7.3');
...
exportgraphics(fig, 'ResNet_Test_ConfusionMatrix_ten.png', 'Resolution', 300);

% save in case want to plot training progress
% save('resnet_training_log_ten.mat', 'infoResNet');
...
% save('RICA_Features_checkpoint_ten.mat','trainingFeatures_RICA', 'validationFeatures_RICA', 'testFeatures_RICA', '-v7.3');

% pre-processing for BiLSTM- participant into cells
...
layers = [ ...
    ...
    bilstmLayer(64, OutputMode="sequence", RecurrentWeightsInitializer="he")
    dropoutLayer(0.3)
    bilstmLayer(20, OutputMode="sequence", RecurrentWeightsInitializer="he")
    dropoutLayer(0.5)
    fullyConnectedLayer(numClasses, WeightsInitializer="he")
    softmaxLayer
];
netLSTM = dlnetwork(layers);
% add l2 regularisation back in
...
save('BiLSTM_training_log.mat', 'info_BiLSTM');
...
true_raw_val = string(validationLabels_flat(:));
model_raw_val = string(predictedLabels_validation(:));
results_table = table(true_raw_val, model_raw_val, 'VariableNames', {'true_labels', 'model_preds'});
writetable(results_table, 'raw_pred_val_ten.csv');
...
true_raw_test = string(testLabels_flat(:));
model_raw_test = string(predictedLabels_test(:));
results_table = table(true_raw_test, model_raw_test, 'VariableNames', {'true_labels', 'model_preds'});
writetable(results_table, 'raw_pred_test_ten.csv');
...
writetable(results_table, 'final_pred_val_ten.csv');
...
writetable(results_table, 'final_pred_test_ten.csv');
