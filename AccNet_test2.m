%% Data Loading  -2 Layer BiLSTM
clc; clear;
data = imageDatastore("/exports/eddie/scratch/s2190468/capture24/data/GAF_ds/", "IncludeSubfolders",true);
%retrieve file names
...
load('ResNet_Trained_Model_ds.mat', 'ResNet_FineTuned')
...
%save('ResNet_Trained_Model_ds.mat', 'ResNet_FineTuned');
...
% confusionchart(test.Labels, YPred, Title="ResNet Test downsample Confusion Matrix");
% 
% %save and do not display in eddie
% exportgraphics(fig, 'ResNet_Test_ConfusionMatrix_ds.png', 'Resolution', 300);

% save in case want to plot training progress
% save('resnet_training_log_ds.mat', 'infoResNet');
...
Mdl = rica(trainingFeatures_ResNet,q,'IterationLimit',2500); % original iteration limit was 80
...
save('RICA_Features_checkpoint_ds.mat','trainingFeatures_RICA', 'validationFeatures_RICA', 'testFeatures_RICA', '-v7.3');
...
layers = [ ...
    ...
    bigruLayer(30, OutputMode="sequence", RecurrentWeightsInitializer="he")
    dropoutLayer(0.2)
    bigruLayer(15, OutputMode="sequence", RecurrentWeightsInitializer="he")
    dropoutLayer(0.3)
...
options = trainingOptions('adam', ...
    ...
    ValidationPatience=15,... %if validation loss goes without improving for 15 checks -> early stopping
...
% savig traininglogs
save('BiLSTM_training_log_test2.mat', 'info_BiLSTM');
...
writetable(results_table, 'final_pred_val_test2.csv');
...
writetable(results_table, 'final_pred_test_test2.csv');
